...
import com.gaestalt.contact.grpc.*;
...
public class SearchComparisonTest {
    ...
    private static void runTest(List<ContactServiceGrpc.ContactServiceBlockingStub> stubs,
        ...
        for (int i = 0; i < totalRequests; i++) {
            ...
            executor.submit(() -> {
                try {
                    ...
                    SearchContactsRequest request = SearchContactsRequest.newBuilder()
                            .setLastName(lastNames[localRandom.nextInt(lastNames.length)])
                            .setMaxResults(25)
                            .setIncludeTotalCount(false)
                            .build();
                    stubs.get(threadIndex).searchContacts(request);
